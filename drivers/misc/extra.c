...
static ssize_t extra_tweaks_version(struct device * dev, struct device_attribute * attr, char * buf)
{
    return sprintf(buf, "%u\n", EXTRA_TWEAKS_VERSION);
}
...
static DEVICE_ATTR(version, S_IRUGO , extra_tweaks_version, NULL);

static struct attribute *extra_tweaks_attributes[] = 
    {
...
static struct attribute_group extra_tweaks_group = 
    {
	.attrs  = extra_tweaks_attributes,
    };

static struct miscdevice extra_tweaks_device = 
    {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "extra_tweaks",
    };

static int __init extra_tweaks_init(void)
{
    int ret;

    pr_info("%s misc_register(%s)\n", __FUNCTION__, extra_tweaks_device.name);

    ret = misc_register(&extra_tweaks_device);
    ...
    if (sysfs_create_group(&extra_tweaks_device.this_device->kobj, &extra_tweaks_group) < 0) 
	{
	    pr_err("%s sysfs_create_group fail\n", __FUNCTION__);
	    pr_err("Failed to create sysfs group for device (%s)!\n", extra_tweaks_device.name);
	}

    return 0;
}

device_initcall(extra_tweaks_init);
